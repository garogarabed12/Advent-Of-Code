...
let getValue [itemType] =
    match priorities |> List.tryFind (fun (k, v) -> k = itemType) with
    | Some (k, v) -> v
    | None -> failwith "Key not found" // generated by chatGPT

let splitCompartments (toSplit: string) =
    let compartment1 = toSplit.Substring(0, toSplit.Length / 2)
    let compartment2 = toSplit.Substring(toSplit.Length / 2)
...
let resultPart1 = input
                    |> Array.map(fun compartment -> splitCompartments(compartment))
